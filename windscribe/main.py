...
class Windscribe:
    ...
    def _run(self, args):
        ...
        lines = list(map(self._parse_line, child.readlines()))
        return lines
    ...
    def connect(self, location):
        lines = self._run(["connect", location])
        print("\n".join(lines))

    def disconnect(self):
        lines = self._run(["disconnect"])
        print("\n".join(lines))

    def firewall(self, mode: str = None):
        if mode:
            lines = self._run(["firewall", mode])
            print("\n".join(lines))
            return

        lines = self._run(["firewall"])
        if len(lines) < 8:
            print("\n".join(lines))
            raise StdoutException
    ...
    def lanbypass(self, mode: str = None):
        if mode:
            lines = self._run(["lanbypass", mode])
            print("\n".join(lines))
            return

        lines = self._run(["lanbypass"])
        if len(lines) < 6:
            print("\n".join(lines))
            raise StdoutException
    ...
    def locations(self):
        ...
        locations = []
        for line in lines:
            location = Location(*self._parse_location(line))
    ...
    def login(self, username: str = None, password: str = None):
        ...
        if logged_in == 0:
            ...
            print("\n".join(self._parse_line(line) for line in child.readlines()))
            return

        raise RuntimeError("Unexpected error")

    def logout(self):
        lines = self._run(["logout"])
        print("\n".join(lines))
